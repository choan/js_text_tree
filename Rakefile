...
APP_VERSION  = '0.0.2'
APP_NAME     = 'TextTree'
APP_BASE_NAME = 'text_tree'
RUBYFORGE_PROJECT = APP_BASE_NAME
APP_TEMPLATE = "#{APP_BASE_NAME}.js.erb"
APP_FILE_NAME= "#{APP_BASE_NAME}.js"
...
task :dist do
  ...
  Dir.chdir(APP_DIST_DIR) do
    FileUtils.copy_file APP_FILE_NAME, "#{APP_BASE_NAME}-#{APP_VERSION}.js"
  end
  if File.directory?("website")
    FileUtils.mkdir_p "website/dist"
    FileUtils.copy_file "dist/#{APP_FILE_NAME}",       "website/dist/#{APP_FILE_NAME}"
    FileUtils.copy_file "dist/#{APP_FILE_NAME}",       "website/dist/#{APP_BASE_NAME}-#{APP_VERSION}.js"
  end
end

Rake::PackageTask.new(APP_BASE_NAME, APP_VERSION) do |package|
...
task :clean_package_source do
  rm_rf File.join(APP_PKG_DIR, "#{APP_BASE_NAME}-#{APP_VERSION}")
end
